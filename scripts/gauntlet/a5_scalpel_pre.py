...
def main() -> None:
    ...
    ck = load_checkpoint("A5")
    if ck is not None:
        baseline_gens = ck.get("baseline_generations", [])
        intervened_gens = ck.get("intervened_generations", [])
        print(f"[A5] resuming: {len(baseline_gens)} baseline + {len(intervened_gens)} intervened done")
    else:
    ...
    for pi, prompt in enumerate(prompts):
        ...
        if any_new:
            save_checkpoint("A5", baseline_gens, intervened_gens)
        if (pi + 1) % 3 == 0:
